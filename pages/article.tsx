...
type ArticlePosts = QueryDatabaseResponse['results']

const Article: NextPage<{ posts: ArticlePosts }> = ({ posts }) => {
  ...
  return (
    <div>
      <Head>
        <title>Hurley Wong - Article</title>
      ...
      <div className="flex flex-col min-h-screen dark:bg-dark-900">
        ...
        <main className="container flex flex-col mx-auto flex-1 max-w-3xl px-6">
          ...
          {posts.map((post: any) => (
            <Link key={post.id} href={`/article/${post.properties.slug.rich_text[0].text.content}`} passHref>
...
export const getStaticProps: GetStaticProps = async () => {
  const db = await getArticleDatabase()
  return {
    props: {
      posts: db,
    },
