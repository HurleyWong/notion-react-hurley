...
import { getNotesDatabase } from '../lib/notion'
import { Search } from 'react-feather'

type NotesPosts = QueryDatabaseResponse['results']

const Notes: NextPage<{ posts: NotesPosts }> = ({ posts }) => {
    ...
    <div>
      ...
      <div className="flex flex-col min-h-screen dark:bg-dark-900">
        ...
        <main className="container flex flex-col mx-auto flex-1 max-w-3xl px-6">
          ...
          {posts.map((post: any) => (
            <Link key={post.id} href={`/notes/${post.properties.slug.rich_text[0].text.content}`} passHref>
