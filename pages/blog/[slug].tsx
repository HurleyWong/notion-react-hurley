...
import { getBlogDatabase, getPage, getBlocks } from '../../lib/notion'
...
const Post: NextPage<{ page: any; blocks: any[] }> = ({ page, blocks }) => {
  ...
  return (
    <div>
      <Head>
        <title>{page.properties.name.title[0].plain_text} - Hurley&apos;s Blog</title>
...
export const getServerSideProps: GetServerSideProps = async ({ params, res }) => {
  ...
  const db = await getBlogDatabase(slug)
  const post = db[0].id
